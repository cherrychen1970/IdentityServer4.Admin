...
namespace SkorubaIdentityServer4Admin.Admin.EntityFramework.PostgreSQL.Extensions
{
    public static class DatabaseExtensions
    {
        ...
        public static void RegisterNpgSqlDbContexts<AdminIdentityDbContext, IdentityServerConfigurationDbContext,
            ...
            where IdentityServerDataProtectionDbContext : DbContext, IDataProtectionKeyContext
        {
            ...
            if (!string.IsNullOrEmpty(dataProtectionConnectionString))
                services.AddDbContext<IdentityServerDataProtectionDbContext>(options => options.UseNpgsql(dataProtectionConnectionString, sql => sql.MigrationsAssembly(migrationsAssembly)));
        }
            ...
            where IdentityServerDataProtectionDbContext : DbContext, IDataProtectionKeyContext
        {
            ...
            // Config DB for identity
            services.AddDbContext<AdminIdentityDbContext>(options => options.UseNpgsql(identityConnectionString, sql => sql.MigrationsAssembly(migrationsAssembly)));

            // Config DB from existing connection
            services.AddConfigurationDbContext<IdentityServerConfigurationDbContext>(options => options.ConfigureDbContext = b => b.UseNpgsql(configurationConnectionString, sql => sql.MigrationsAssembly(migrationsAssembly)));

            // Operational DB from existing connection
            services.AddOperationalDbContext<IdentityServerPersistedGrantDbContext>(options => options.ConfigureDbContext = b => b.UseNpgsql(persistedGrantConnectionString, sql => sql.MigrationsAssembly(migrationsAssembly)));
